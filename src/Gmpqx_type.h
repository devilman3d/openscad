...
#ifndef CGAL_GMPQX_TYPE_H
#define CGAL_GMPQX_TYPE_H

#ifdef CGAL_GMPQ_TYPE_H
#error "CGAL/Gmpq_type.h is already included"
#endif

#define CGAL_GMPQ_TYPE_H

#include <CGAL/basic.h>
#include "Gmpzx_type.h"
#include "Gmpfrx_type.h"
...
struct Gmpq_rep
{
	...
	virtual ~Gmpq_rep() { mpq_clear(mpQ); }

private:
	// Make sure it does not get accidentally copied.
	Gmpq_rep(const Gmpq_rep &) = delete;
	Gmpq_rep & operator= (const Gmpq_rep &) = delete;
};
...
#ifdef CGAL_GMPQ_NO_REFCOUNT
  Gmpq(const Gmpq &q) : Gmpq_rep()
  {
	  Gmp_lock lock(&q);
	  mpq_set(mpq(), q.mpq());
  }
...
// the same as Quotient<MP_Float>
  ...
  bool operator==(const Gmpq &q) const {
	  Gmp_lock lock(this, &q);
	  return mpq_equal(this->mpq(), q.mpq()) != 0;
  }
  bool operator< (const Gmpq &q) const {
	  Gmp_lock lock(this, &q);
	  return mpq_cmp(this->mpq(), q.mpq()) < 0; 
  }

  double to_double() const;
  Sign sign() const;

  const mpq_t &mpq() const {
#ifndef CGAL_GMPQ_NO_REFCOUNT
	  return Ptr()->mpQ;
#else
	  return this->mpQ;
  ...
  Gmpq& operator+=(long z);
  Gmpq& operator-=(long z);
  Gmpq& operator*=(long z);
  Gmpq& operator/=(long z);
  bool  operator==(long z) const {
	  Gmp_lock lock(this);
	  return mpq_cmp_si(mpq(), z, 1) == 0;
  }
  bool  operator< (long z) const {
  ...
  Gmpq& operator+=(const Gmpfr &f);
  Gmpq& operator-=(const Gmpfr &f);
  Gmpq& operator*=(const Gmpfr &f);
  Gmpq& operator/=(const Gmpfr &f);
  bool  operator==(const Gmpfr &f) const {
	  Gmp_lock lock(this, &f);
	  return mpfr_cmp_q(f.fr(), mpq()) == 0;
  }
  bool  operator< (const Gmpfr &f) const {
	  Gmp_lock lock(this, &f);
	  return mpfr_cmp_q(f.fr(), mpq()) > 0;
...
inline
Gmpq&
Gmpq::operator/=(const Gmpq &y)
{
	CGAL_precondition(y != 0);
	Gmpq Res;
	Gmp_lock lock(this, &y);
	mpq_div(Res.mpq(), mpq(), y.mpq());
    Res.swap(*this);
    return *this;
}

inline
Gmpq& Gmpq::operator+=(int z) {
	Gmp_lock lock(this);
	*this += Gmpq(z);
	return *this;
}
...
inline
Gmpq& Gmpq::operator+=(long long z) {
	Gmp_lock lock(this);
	*this += Gmpq(z);
	return *this;
}
...
inline
Gmpq& Gmpq::operator+=(const Gmpfr &f) {
	Gmp_lock lock(this, &f);
	*this += Gmpq(f);
	return *this;
}
