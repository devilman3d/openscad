...
class ColorFactoryNode : public ColorNode
{
public:
	static std::unordered_map<std::string, Color4f> webcolors;

	ColorFactoryNode() : ColorNode("c", "alpha") { }
	...
	void initialize(Context &c, const ModuleContext &evalctx) override
	{
		...
		if (v->type() == Value::VECTOR) {
			for (size_t i = 0; i < 4; i++) {
				this->color[i] = i < v->toVector().size() ? v->toVector()[i]->toDouble() : 1.0;
				if (this->color[i] > 1)
					PRINTB_NOCACHE("WARNING: color() expects numbers between 0.0 and 1.0. Value of %.1f is too large.", this->color[i]);
			}
		}
		...
		if (alpha->type() == Value::NUMBER) {
			this->color[3] = alpha->toDouble();
		}
	}
};

FactoryModule<ColorFactoryNode> ColorFactory("color");

// Colors extracted from https://drafts.csswg.org/css-color/ on 2015-08-02
// CSS Color Module Level 4 - Editor’s Draft, 29 May 2015
std::unordered_map<std::string, Color4f> ColorFactoryNode::webcolors = map_list_of
