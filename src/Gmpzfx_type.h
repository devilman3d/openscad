...
#ifndef CGAL_GMPZFX_TYPE_H
#define CGAL_GMPZFX_TYPE_H

#ifdef CGAL_GMPZF_TYPE_H
#error CGAL_GMPZF_TYPE_H already defined
#endif
...
// But then we should use the Storage_traits::construct/get...
  ...
  Gmpzf_rep() {}
  virtual ~Gmpzf_rep() { mpz_clear(mpZ); }
...
#ifdef CGAL_GMPZF_NO_REFCOUNT
  Gmpzf(const Gmpzf &z)
	  : Gmpzf_rep()
	  , e(0)
  {
	  Gmp_lock lock(&z);
	  mpz_init_set(man(), z.man());
	  canonicalize();
  }

  Gmpzf &operator=(const Gmpzf &z)
  {
...
Gmpzf& Gmpzf::operator+=( const Gmpzf& b)
{
  ...
  if (!b.is_zero()) { // important in sparse contexts
	  Gmpzf result;
...
void Gmpzf::align ( const mpz_t*& a_aligned,
  ...
  static thread_local Gmpz s;
  Gmp_lock::assert_locked(&a, &b);
